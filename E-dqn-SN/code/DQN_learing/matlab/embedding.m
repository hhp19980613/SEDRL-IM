...
% function f = init_embed(graphName,row)
    f = 1;

    disp(['embedding beginning ']);

    load('..//data//embedding_in.mat');
% ================================= 璇诲彇鏁版嵁 =======================================


    nodeNum = size(matrix,1);

    matrix_VN = getMatrixVN(matrix,nodeNum,idx);


    G=graph(matrix_VN);
    nodeNum_temp = size(matrix_VN,1);


    embedSize = 64;
    nnsize = [nodeNum_temp 512 256 128 embedSize];        %layer-wised setting
    ...
    embedInfo = rep2{1,4};
    save('..//data//embedding_out.mat','embedInfo');
    disp(['The embedding of nodes finish']);
end
% ============================end of init_embed()=====================================
