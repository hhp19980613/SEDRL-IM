...
for i = 1 : numel(sae.ae)  %规定每一层的神经网络的相关参数？
    %disp(['Training SAE ' num2str(i) '/' num2str(numel(sae.ae))]);
    opts.batchsize = 700; %process how many instances in each batch , community detection,训练数据每一块大小
    
    if i==1
        opts.numepochs = 100;  %重复训练次数 
        alfa=alfa1;
    else
        opts.numepochs =50;
